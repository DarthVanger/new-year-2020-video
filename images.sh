...
ffmpeg -y -i  output/video.mp4 -i output/bezviz.jpg \
    -filter_complex "[0:v][1:v] overlay=0:0:enable='between(t,122.25,122.5)'" \
     output/temp2.mp4

ffmpeg -y -i output/temp2.mp4 -i output/grad.mp4 \
    -filter_complex "[0:v][1:v] overlay=0:0:enable='between(t,122.5,122.75)'" \
     output/temp3.mp4

ffmpeg -y -i output/temp3.mp4 -i output/ussr.jpg \
    -filter_complex "[0:v][1:v] overlay=0:0:enable='between(t,123,123.25)'" \
     output/temp4.mp4

ffmpeg -y -i output/temp4.mp4 -i output/bandera.jpg \
    -filter_complex "[0:v][1:v] overlay=0:0:enable='between(t,123.25,123.5)'" \
     output/video_t.mp4
