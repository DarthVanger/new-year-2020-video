...
ffmpeg -y -ss 135 -i output/video.mp4 output/07.mp4
...
ffmpeg -y -i output/06.mp4 -i output/berlin2.mp4 \
    -filter_complex "[0:v][1:v] blend=all_mode='overlay'" \
    output/b05-1.mp4

ffmpeg -y -i output/b05-1.mp4 -i output/berlin2.mp4 \
    -filter_complex "[0:v] format=rgba [a]; [1:v] format=rgba [b]; [a][b] blend=all_expr='A*if(gte(X,440/2.66), 1, 0)*if(lte(X,840/2.66), 1, 0) + B*if(gte(X,440/2.66), 0, 1) + B*if(lte(X,840/2.66), 0, 1)'" \
    output/b05.mp4


ffmpeg -y -i output/07.mp4 -i output/maidan.mp4 \
    -filter_complex "[0:v][1:v] blend=all_mode='overlay'" \
    output/b06-1.mp4

ffmpeg -y -i output/b06-1.mp4 -i output/maidan.mp4 \
    -filter_complex "[0:v] format=rgba [a]; [1:v] format=rgba [b]; [a][b] blend=all_expr='A*if(gte(X,440/2.66), 1, 0)*if(lte(X,840/2.66), 1, 0) + B*if(gte(X,440/2.66), 0, 1) + B*if(lte(X,840/2.66), 0, 1)'" \
    output/b06.mp4
